...
void test_digits_file(void) {
  ...
  if (mpc_parse_contents("tests/mpc/digits.txt", Program, &r)) {
    PT_ASSERT(1);
  ...
  f = fopen("tests/mpc/digits.txt", "r");
  PT_ASSERT(f != NULL);

  if (mpc_parse_file("tests/mpc/digits.txt", f, Program, &r)) {
    PT_ASSERT(1);
    PT_ASSERT(mpc_ast_eq(t0, r.output));
    mpc_ast_delete(r.output);
  } else {
  ...
  if (mpc_parse("tests/mpc/digits.txt", "123", Program, &r)) {
    PT_ASSERT(1);
    PT_ASSERT(mpc_ast_eq(t0, r.output));
    mpc_ast_delete(r.output);
  } else {
